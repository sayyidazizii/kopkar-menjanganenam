...
class PersonaliaReport_model extends CI_Model
{
	...
	public function getPersonaliaReport2($savings_cash_mutation_date, $savings_cash_mutation_status)
	{
		$this->db->select('acct_savings_cash_mutation.savings_cash_mutation_id, acct_savings_cash_mutation.savings_account_id, acct_savings_cash_mutation.member_id, acct_savings_cash_mutation.savings_id, acct_savings_cash_mutation.branch_id, acct_savings_cash_mutation.mutation_id, acct_savings_cash_mutation.savings_cash_mutation_date, acct_savings_cash_mutation.savings_cash_mutation_amount, acct_savings_cash_mutation.savings_cash_mutation_remark, acct_savings_cash_mutation.savings_cash_mutation_status, acct_savings_cash_mutation.operated_name, acct_mutation.mutation_name');
		$this->db->from('acct_savings_cash_mutation');
		$this->db->join('acct_mutation', 'acct_savings_cash_mutation.mutation_id = acct_mutation.mutation_id');
	...
	public function getCreditDetail( $month_name, $years)
	{
		$this->db->select('core_member.member_id, core_member.member_name, acct_credits_account.credits_account_id, acct_credits_account.credits_account_interest_last_balance, acct_credits_account.credits_account_serial');
	...
	public function getCreditPaymentInterest($month_name, $years, $credits_account_id)
	{
		$this->db->select('SUM(acct_credits_payment.credits_payment_interest) AS credits_payment_principal, SUM(acct_credits_payment.credits_payment_principal) AS credits_payment_interest');
		$this->db->from('acct_credits_payment');
	...
	public function getAcctSavingsAccount($month_name, $years)
	{
		...
		$this->db->where('MONTH(acct_savings_account.savings_account_date)',  $month_name);
		$this->db->where('YEAR(acct_savings_account.savings_account_date)',  $years);
		$result = $this->db->get()->result_array();
		return $result;
	}
	public function getCoreMember($month_name, $years)
	{
		...
		$this->db->where('MONTH(core_member.member_register_date)', $month_name);
		$this->db->where('YEAR(core_member.member_register_date) ', $years);
		return $this->db->get()->result_array();
	}
