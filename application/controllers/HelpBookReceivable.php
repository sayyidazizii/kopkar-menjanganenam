...
<?php
	...
	Class HelpBookReceivable extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookReceivable_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookReceivable_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']					= 'HelpBookReceivable/HelpBookReceivable_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookReceivable_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookReceivable_model->getHelpBookReceivableDebit($sesi);

			$data_kredit 	= $this->HelpBookReceivable_model->getHelpBookReceivableKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU PIUTANG LAIN - LAIN</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Piutang Lain - Lain")
									->setSubject("")
									->setDescription("Buku Pembantu - Piutang Lain - Lain")
									->setKeywords("Buku Pembantu - Piutang Lain - Lain")
									->setCategory("Buku Pembantu - Piutang Lain - Lain");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - PIUTANG LAIN - LAIN");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookReceivable_model->getHelpBookReceivablePelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Piutang Lain - Lain.xls';
