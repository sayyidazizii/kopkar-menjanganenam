<?php
	Class AcctJournalPPOB extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('AcctJournalPPOB_model');
		...
		public function index(){
			...
			$sesi	= 	$this->session->userdata('filter-AcctJournalPPOB');
			...
			$data['main_view']['corebranch']			= create_double($this->AcctJournalPPOB_model->getCoreBranch(),'branch_id','branch_name');
			$data['main_view']['AcctJournalPPOB']		= $this->AcctJournalPPOB_model->getAcctJournalPPOB($sesi['start_date'], $sesi['end_date'], $sesi['branch_id']);	

			$data['main_view']['content']				= 'AcctJournalPPOB/ListJournalPPOBNew_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-AcctJournalPPOB',$data);
			redirect('AcctJournalPPOB');
		}

		public function printNoteAcctJournalPPOB(){
			$auth = $this->session->userdata('auth');
			$savings_account_id = $this->uri->segment(3);
			$acctsavingsaccount	= $this->AcctJournalPPOB_model->getAcctJournalPPOB_Detail($savings_account_id);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			        ...
			        <td width=\"30%\"><div style=\"text-align: center;\">".$this->AcctJournalPPOB_model->getBranchCity($auth['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
		...
		public function validationAcctJournalPPOB(){
			$auth = $this->session->userdata('auth');
			$savings_account_id = $this->uri->segment(3);
			...
			if($this->AcctJournalPPOB_model->validationAcctJournalPPOB($data)){
				$msg = "<div class='alert alert-success alert-dismissable'>  
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
							Validasi Rekening Sukses
						</div>";
				$this->session->set_userdata('message',$msg);
				redirect('AcctJournalPPOB/printValidationAcctJournalPPOB/'.$savings_account_id);
			}else{
		...
		public function printValidationAcctJournalPPOB(){
			$savings_account_id = $this->uri->segment(3);
			$acctsavingsaccount	= $this->AcctJournalPPOB_model->getAcctJournalPPOB_Detail($savings_account_id);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    ...
			    <tr>
			        ...
			        <td width=\"18%\"><div style=\"text-align: right; font-size:14px\">".$this->AcctJournalPPOB_model->getUsername($acctsavingsaccount['validation_id'])."</div></td>
		...
		public function voidAcctJournalPPOB(){
			...
			$data['main_view']['acctsavingsaccount']		= $this->AcctJournalPPOB_model->getAcctJournalPPOB_Detail($this->uri->segment(3));
			$data['main_view']['content']			= 'AcctJournalPPOB/FormVoidAcctJournalPPOB_view';
			$this->load->view('MainPage_view',$data);
		}

		public function processVoidAcctJournalPPOB(){
			$auth	= $this->session->userdata('auth');
			...
			if($this->form_validation->run()==true){
				if($this->AcctJournalPPOB_model->voidAcctJournalPPOB($newdata)){
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
