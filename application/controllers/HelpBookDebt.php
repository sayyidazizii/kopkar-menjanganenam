...
<?php
	...
	Class HelpBookDebt extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookDebt_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookDebt_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']					= 'HelpBookDebt/HelpBookDebt_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookDebt_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookDebt_model->getHelpBookDebtDebit($sesi);

			$data_kredit 	= $this->HelpBookDebt_model->getHelpBookDebtKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU HUTANG LAIN - LAIN</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Hutang Lain - Lain")
									->setSubject("")
									->setDescription("Buku Pembantu - Hutang Lain - Lain")
									->setKeywords("Buku Pembantu - Hutang Lain - Lain")
									->setCategory("Buku Pembantu - Hutang Lain - Lain");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - HUTANG LAIN - LAIN");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookDebt_model->getHelpBookDebtPelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Hutang Lain - Lain.xls';
