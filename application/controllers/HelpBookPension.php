...
<?php
	...
	Class HelpBookPension extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookPension_model');
		...
		public function index(){
			$corebranch 								= create_double_branch($this->HelpBookPension_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']				= 'HelpBookPension/HelpBookPension_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookPension_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookPension_model->getHelpBookPensionDebit($sesi);

			$data_kredit 	= $this->HelpBookPension_model->getHelpBookPensionKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU HUTANG DANA PENSIUN</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Hutang Pensiun")
									->setSubject("")
									->setDescription("Buku Pembantu - Hutang Pensiun")
									->setKeywords("Buku Pembantu - Hutang Pensiun")
									->setCategory("Buku Pembantu - Hutang Pensiun");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - HUTANG PENSIUN");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookPension_model->getHelpBookPensionPelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Hutang Pensiun.xls';
