...
<?php
	...
	Class HelpBookEmployee extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookEmployee_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookEmployee_model->getCoreBranch(),'branch_id','branch_name');

			$data['main_view']['content']					= 'HelpBookEmployee/HelpBookEmployee_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookEmployee_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookEmployee_model->getHelpBookEmployeeDebit($sesi);

			$data_kredit 	= $this->HelpBookEmployee_model->getHelpBookEmployeeKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU UANG MUKA KARYAWAN</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Uang Muka Karyawan")
									->setSubject("")
									->setDescription("Buku Pembantu - Uang Muka Karyawan")
									->setKeywords("Buku Pembantu - Uang Muka Karyawan")
									->setCategory("Buku Pembantu - Uang Muka Karyawan");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - UANG MUKA KARYAWAN");
			...
			$this->excel->getActiveSheet()->setCellValue('K7', "Umur Uang Muka (Hari)");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookEmployee_model->getHelpBookEmployeePelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Uang Muka Karyawan.xls';
