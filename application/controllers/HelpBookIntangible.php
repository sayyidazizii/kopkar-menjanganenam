...
<?php
	...
	Class HelpBookIntangible extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookIntangible_model');
		...
		public function index(){
			$corebranch 					= create_double_branch($this->HelpBookIntangible_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']	= 'HelpBookIntangible/HelpBookIntangible_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookIntangible_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookIntangible_model->getHelpBookIntangibleDebit($sesi);

			$data_kredit 	= $this->HelpBookIntangible_model->getHelpBookIntangibleKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU ASET TIDAK BERWUJUD</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Aset Tidak Berwujud")
									->setSubject("")
									->setDescription("Buku Pembantu - Aset Tidak Berwujud")
									->setKeywords("Buku Pembantu - Aset Tidak Berwujud")
									->setCategory("Buku Pembantu - Aset Tidak Berwujud");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - ASET TIDAK BERWUJUD");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookIntangible_model->getHelpBookIntangiblePelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Aset Tidak Berwujud.xls';
