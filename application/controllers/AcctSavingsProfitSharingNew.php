				...
				if(empty($data_log_step4)){	
					...
					foreach($acctsavingsaccount as $krey => $vral){
						if($vral['savings_id'] == 35){
							$profitsharing	= $vral['savings_account_interest_rate']*12;
						}else{
						...
						$savings_interest_temp_amount 			= round(($savings_account_daily_average_balance * ($profitsharing / 12)) / 100);
						...
						if($savings_interest_temp_amount > $preferencecompany['tax_minimum_amount']){
							$savings_tax_temp_amount = round($savings_interest_temp_amount * $preferencecompany['tax_percentage'] / 100);
						}else{
						...
						if($member_id != $vral['member_id']){
							//menambahkan bunga simpanan berjangka pada total bunga utk hitungan jasa nnti
							foreach($depositoprofitsharing as $dkey => $dval){
								$tempdata['savings_interest_temp_amount'] 		= $tempdata['savings_interest_temp_amount'] + $dval['deposito_profit_sharing_amount'];
								$tempdata['savings_profit_sharing_temp_amount'] = $tempdata['savings_profit_sharing_temp_amount'] + $dval['deposito_profit_sharing_amount'];
								$tempdata['savings_profit_sharing_tax_paid'] 	= $tempdata['savings_profit_sharing_tax_paid'] + $dval['deposito_profit_sharing_tax'];
							}
							//close tambah bunga simpanan berjangka
							...
							$tempdata = array(
								...
								'savings_profit_sharing_tax_paid'			=> 0,
							...
							$tempdata = array(
								...
								'savings_profit_sharing_tax_paid'			=> $tempdata['savings_profit_sharing_tax_paid'],
								'savings_profit_sharing_temp_token'			=> $savings_profit_sharing_period.$vral['savings_account_id'],
								'operated_name'								=> 'SYSTEM',
								'created_id'								=> $auth['user_id'],
								'created_on'								=> date('Y-m-d H:i:s'),
						...
						$member_id 				= $vral['member_id'];
						$depositoprofitsharing  = $this->AcctSavingsProfitSharingNew_model->getDepositoProfitSharing($member_id, $data['month_period'], $data['year_period']); //mengambil list bunga deposito sebelum id anggota diubah
					}

					//untuk data terakhir dari looping diatas
					...
					$tempdata['savings_tax_temp_amount'] = $savings_tax_temp_amount;
					array_push($arraydata, $tempdata);
					//close utk data terakhir looping atas
					...
					foreach ($acctsavingsaccount as $k => $v) {
						if($v['savings_id'] == 35){
							$profitsharing	= $v['savings_account_interest_rate']*12;
						}else{
					...
					if($this->AcctSavingsProfitSharingNew_model->insertAcctSavingsProfitSharingFix()){
						...
						foreach ($corebranch as $key => $vCB) {
							...
							foreach($savingsprofitsharingtotaltemp as $key => $val){
								//tax EOM dari akumulasi dikurangi tax deposito yg sudah diproses
								$totaltax 		= $val['savings_tax_temp_amount'] - $val['savings_profit_sharing_tax_paid'];
