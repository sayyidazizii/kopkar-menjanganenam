<?php 
	...
	ob_start();?>
<?php
	Class AcctConsolidationReportNew extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('AcctConsolidationReportNew_model');
		...
		public function AcctBalanceSheet(){
			...
			$branch_name 					= $this->AcctConsolidationReportNew_model->getBranchName($preferencecompany['central_branch_id']);
			...
			// $data['main_view']['corebranch']					= create_double($this->AcctConsolidationReportNew_model->getCoreBranch(),'branch_id','branch_name');

			$data['main_view']['corebranch']					= $corebranch;

			$data['main_view']['acctbalancesheetreport_left']	= $this->AcctConsolidationReportNew_model->getAcctBalanceSheetReport_Left();

			$data['main_view']['acctbalancesheetreport_right']	= $this->AcctConsolidationReportNew_model->getAcctBalanceSheetReport_Right();
			$data['main_view']['content']						= 'AcctConsolidationReportNew/AcctBalanceSheetConsolidationReport_view';
		...
		public function filterAcctBalanceSheet(){
			...
			redirect('AcctConsolidationReportNew/AcctBalanceSheet');
		}
		...
		public function processPrintingAcctBalanceSheet(){
			...
			if($sesi['consolidation_report'] == 0 || $sesi['consolidation_report'] == 9){
				$branch_name = $this->AcctConsolidationReportNew_model->getBranchName($sesi['branch_id']);
			} else {
			...
			$acctbalancesheetreport_left	= $this->AcctConsolidationReportNew_model->getAcctBalanceSheetReport_Left();

			/*print_r("acctbalancesheetreport_left ");
			print_r($acctbalancesheetreport_left);
			exit;*/

			$acctbalancesheetreport_right	= $this->AcctConsolidationReportNew_model->getAcctBalanceSheetReport_Right();
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctbalancesheetreport_left as $keyLeft => $valLeft) {
									...
									if($valLeft['report_type1']	== 3){
										...
										$acctaccountlist 	= $this->AcctConsolidationReportNew_model->getAcctAccount_List($length, $valLeft['account_code1']);
										...
										if(!empty($acctaccountlist)){
											foreach ($acctaccountlist as $ka => $va){
												$last_balance 	= $this->AcctConsolidationReportNew_model->getLastBalance($va['account_id'], $sesi['consolidation_report'], $sesi['branch_id'], $last_month, $last_year);		
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctbalancesheetreport_right as $keyRight => $valRight) {
									...
									if($valRight['report_type2']	== 3){
										...
										$acctaccountlist 	= $this->AcctConsolidationReportNew_model->getAcctAccount_List($length, $valRight['account_code2']);
									...
									if($valRight['report_type2']	== 8){
										...
										$sahu_tahun_lalu = $this->AcctConsolidationReportNew_model->getSHUTahunLalu($sesi['branch_id'], $month, $year);

										if(empty($sahu_tahun_lalu)){
											$sahu_tahun_lalu = 0;
										}

										$pembagian_shu_tahun_lalu	= $this->AcctConsolidationReportNew_model->getPembagianSHUTahunLalu($sesi['branch_id'], $month, $year);
									...
									if($valRight['report_type2'] == 7){
										$profit_loss = $this->AcctConsolidationReportNew_model->getProfitLossAmount($sesi['branch_id'], $month, $year);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			        ...
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px\">".$this->AcctConsolidationReportNew_model->getBranchCity($sesi['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
			    ...
			    <tr>
			        ...
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">".strtoupper($this->AcctConsolidationReportNew_model->getBranchManager($sesi['branch_id']))."</div></td>
			    </tr>				
			</table>";
			...
			if(!empty($acctbalancesheetreport_left && $acctbalancesheetreport_right)){
				...
				foreach($acctbalancesheetreport_left as $keyLeft =>$valLeft){
					if(is_numeric($keyLeft)){
						...
						if($valLeft['report_type1']	== 3){
							$last_balance1 = $this->AcctConsolidationReportNew_model->getLastBalance($valLeft['account_id1'], $sesi['consolidation_report'], $sesi['branch_id'], $sesi['month_period'], $sesi['year_period']);		
				...
				foreach($acctbalancesheetreport_right as $keyRight =>$valRight){
					if(is_numeric($keyRight)){
						...
						if($valRight['report_type2']	== 3){
							$last_balance2 = $this->AcctConsolidationReportNew_model->getLastBalance($valRight['account_id2'], $sesi['consolidation_report'], $sesi['branch_id'], $sesi['month_period'], $sesi['year_period']);		
		...
		public function AcctProfitLoss(){
			...
			$data['main_view']['acctprofitlossreport_top']		= $this->AcctConsolidationReportNew_model->getAcctProfitLossReport_Top();

			$data['main_view']['acctprofitlossreport_bottom']	= $this->AcctConsolidationReportNew_model->getAcctProfitLossReport_Bottom();
			...
			$data['main_view']['content']						= 'AcctConsolidationReportNew/AcctProfitLossConsolidationReport_view';
		...
		public function filterAcctProfitLoss(){
			...
			redirect('AcctConsolidationReportNew/AcctProfitLoss');
		}
		...
		public function processPrintingAcctProfitLoss(){
			...
			$preference_company 			= $this->AcctConsolidationReportNew_model->getPreferenceCompany();

			$acctprofitlossreport_top		= $this->AcctConsolidationReportNew_model->getAcctProfitLossReport_Top();

			$acctprofitlossreport_bottom	= $this->AcctConsolidationReportNew_model->getAcctProfitLossReport_Bottom();
		        			...
		        			<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctprofitlossreport_top as $keyTop => $valTop) {
									...
									if($valTop['report_type']	== 3){
										...
										$account_subtotal 	= $this->AcctConsolidationReportNew_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctprofitlossreport_bottom as $keyBottom => $valBottom) {
									...
									if($valBottom['report_type']	== 3){
										...
										$account_subtotal 	= $this->AcctConsolidationReportNew_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			    	<td width=\"40%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px\">".$this->AcctConsolidationReportNew_model->getBranchCity($data['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
			        <td width=\"40%\" height=\"90px\"><div style=\"text-align: center;font-size:12px\">Yang Melaporkan</div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			    ...
			    <tr>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">ADMIN</div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">".strtoupper($this->AcctConsolidationReportNew_model->getBranchManager($data['branch_id']))."</div></td>
			    </tr>				
			</table>";
			...
			if(!empty($acctprofitlossreport_top && $acctprofitlossreport_bottom)){
				...
				foreach($acctprofitlossreport_top as $keyTop => $valTop){
					if(is_numeric($keyTop)){
						...
						if($valTop['report_type']	== 3){
							$accountamount 		= $this->AcctConsolidationReportNew_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
				...
				foreach($acctprofitlossreport_bottom as $keyBottom => $valBottom){
					if(is_numeric($keyTop)){
						...
						if($valBottom['report_type']	== 3){
							$accountamount 		= $this->AcctConsolidationReportNew_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
