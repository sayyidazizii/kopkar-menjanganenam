...
<?php
	...
	Class HelpBookInsurance extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookInsurance_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookInsurance_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']					= 'HelpBookInsurance/HelpBookInsurance_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookInsurance_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookInsurance_model->getHelpBookInsuranceDebit($sesi);

			$data_kredit 	= $this->HelpBookInsurance_model->getHelpBookInsuranceKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU HUTANG ASURANSI PINJAMAN</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Hutang Asuransi Pinjaman")
									->setSubject("")
									->setDescription("Buku Pembantu - Hutang Asuransi Pinjaman")
									->setKeywords("Buku Pembantu - Hutang Asuransi Pinjaman")
									->setCategory("Buku Pembantu - Hutang Asuransi Pinjaman");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - HUTANG ASURANSI PINJAMAN");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookInsurance_model->getHelpBookInsurancePelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Hutang Asuransi Pinjaman.xls';
