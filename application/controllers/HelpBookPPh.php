...
<?php
	...
	Class HelpBookPPh extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookPPh_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookPPh_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']					= 'HelpBookPPh/HelpBookPPh_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookPPh_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookPPh_model->getHelpBookPPhDebit($sesi);

			$data_kredit 	= $this->HelpBookPPh_model->getHelpBookPPhKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU HUTANG PPh 4 (2)</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Hutang PPh 4 (2)")
									->setSubject("")
									->setDescription("Buku Pembantu - Hutang PPh 4 (2)")
									->setKeywords("Buku Pembantu - Hutang PPh 4 (2)")
									->setCategory("Buku Pembantu - Hutang PPh 4 (2)");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - HUTANG PPh 4 (2)");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookPPh_model->getHelpBookPPhPelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Hutang PPh 4 (2).xls';
