...
<?php
	...
	Class HelpBookJamsostek extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('HelpBookJamsostek_model');
		...
		public function index(){
			$corebranch 									= create_double_branch($this->HelpBookJamsostek_model->getCoreBranch(),'branch_id','branch_name');
			
			$data['main_view']['content']					= 'HelpBookJamsostek/HelpBookJamsostek_view';
		...
		public function print($sesi){
			...
			$preferencecompany 	= $this->HelpBookJamsostek_model->getPreferenceCompany();
			...
			$data_debit 	= $this->HelpBookJamsostek_model->getHelpBookJamsostekDebit($sesi);

			$data_kredit 	= $this->HelpBookJamsostek_model->getHelpBookJamsostekKredit($sesi);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"100%\"><div style=\"text-align: center; font-size:14px; font-weight:bold\">BUKU PEMBANTU HUTANG JAMSOSTEK</div></td>
			    </tr>
				<tr>
		...
		public function export($sesi){	
			...
			$this->excel->getProperties()->setCreator("CST FISRT")
									...
									->setTitle("Buku Pembantu - Hutang Jamsostek")
									->setSubject("")
									->setDescription("Buku Pembantu - Hutang Jamsostek")
									->setKeywords("Buku Pembantu - Hutang Jamsostek")
									->setCategory("Buku Pembantu - Hutang Jamsostek");
			...
			$this->excel->getActiveSheet()->setCellValue('B2', "BUKU PEMBANTU - HUTANG JAMSOSTEK");
			...
			$this->excel->getActiveSheet()->setCellValue('K7', "Umur Hutang (Hari)");
			...
			foreach($data_debit as $key => $val){
				...
				$datapelunasan = $this->HelpBookJamsostek_model->getHelpBookJamsostekPelunasan($sesi, $val['journal_voucher_id']);
				if($datapelunasan){ 	
					$this->excel->getActiveSheet()->setCellValue('B'.($row), $no);
			...
			$filename='Buku Pembantu - Hutang Jamsostek.xls';
