...
<script>
    ...
    function hitungPlafon() {
        ...
        $('.sales-item').each(function (index, element) {
            let id = $(this).data('id');
            let sbs = parseInt($('#total_amount_'+id).val());
            if($('#add_sales_invoice_id_' + id).is(":checked")){
                plafon += sbs;
            }
            $('#credits_account_last_balance_principal').val(plafon);
            $('#credits_account_last_balance_principal_view').val(toRp(plafon));
            $('#credits_account_last_balance_principal_view').textbox('setValue',toRp(plafon));
			...
			<div class="portlet box blue">
				...
				<div class="portlet-body">
					<div class="col-md-4">
                        ...
                        <table style="width: 100%;"  class="table table-bordered table-striped" padding:"0">
                            <thead>
                                <th>No</th>
                                <th>Tanggal</th>
                                <th>No Penjualan</th>
                            ...
                            <?php
                                ...
                                ?>
                                <tr class="sales-item" data-id="<?=$value->sales_invoice_id?>">
									<td><?=$no?></td>
									<td><?=date('d-m-Y',strtotime($value->sales_invoice_date))?></td>
									<td><?=$value->sales_invoice_no?></td>
									<td><?=number_format($value->total_amount,2)?>
                                    <input type="hidden" name="sales_invoice[<?=$value->sales_invoice_id?>][total_amount]" id="total_amount_<?=$value->sales_invoice_id?>" value="<?=$value->total_amount?>">
                                    <input type="hidden" name="sales_invoice[<?=$value->sales_invoice_id?>][sales_invoice_id]" id="sales_invoice_id_<?=$value->sales_invoice_id?>" value="<?=$value->sales_invoice_id?>"></td>
									<td><input type="checkbox" onchange="hitungPlafon()" name="sales_invoice[<?=$value->sales_invoice_id?>][add]" id="add_sales_invoice_id_<?=$value->sales_invoice_id?>" value="true"></td>
								</tr>
								<?php
                                    }
                                ?>
								...
								<tr>
									...
									<td>
										<input type="text" class="easyui-textbox"
											name="credits_account_last_balance_principal_view" readonly
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		table = $('#myDataTable2').DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?php echo site_url('credit-account/member-list-sales') ?>",
				"type": "POST",
			},
			"columnDefs": [{
				"targets": [0],
